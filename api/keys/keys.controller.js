...
//check if key exists with active state being true
checkActivekeyByinstitution(body.user, (err, results) => {
  ...
  singlependingKeys: (req, res) => {
    const id = req.params.id;
    getpendingkeyById(id, (err, results) => {
      if (err) {
        console.log(err);
        return;
      }
  ...
  deletependingKey: (req, res) => {
     const data = req.params;
    deletepending(data.req_id ,(err, results) => {
      if (err) {
        console.log(err);
        return res.json({
          success: 0,
